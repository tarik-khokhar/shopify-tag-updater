// api/shopify.js — Vercel Serverless Proxy v4
// Uses query params instead of custom headers — avoids CORS preflight issues
...
module.exports = async function handler(req, res) {
  ...
  // Debug endpoint — visit /api/shopify?debug=1 in browser to check env vars
  if (req.query.debug) {
    res.status(200).json({
  ...
  if (!STORE || !TOKEN) {
    res.status(500).json({
      error: 'Missing env vars — visit /api/shopify?debug=1 to diagnose',
  ...
  // shopifyPath comes from query param: ?p=products.json%3Flimit%3D250
  const shopifyPath   = req.query.p;
  const shopifyMethod = (req.query.m || req.method || 'GET').toUpperCase();
